                   ...
                   // hotelToUpdate.setVille(updatedVille);
        		    ...
        		    if (filterVilleParam != null && !filterVilleParam.isEmpty()) {
        		        try {
        		            ...
        		            if (villeId == 0) {
        		                hotelList = Hotelejb.findAll();
        		            } else {
        		                hotelList = Hotelejb.findByVille(villeId);
        		            }

        		            List<Hotel> villeList = Hotelejb.findAll();
        		    ...
        		    request.getRequestDispatcher("HotelController").forward(request, response);
        		}
            } else {
                String nom = request.getParameter("nom");
                String adresse = request.getParameter("adresse");
